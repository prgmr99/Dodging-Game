...
# 화면 타이틀 설정
pygame.display.set_caption("Yeom Pang")  
...
# 스테이지 만들기
stage=pygame.image.load(os.path.join(image_path, "stage1.jpg"))
...
ball_images=[
    pygame.image.load(os.path.join(image_path, "balloon111.png")),
    pygame.image.load(os.path.join(image_path, "balloon222.png")),
    pygame.image.load(os.path.join(image_path, "balloon333.png")),
...
while running:
    ...
    # 무기 위치 조정 ***
    weapons=[ [w[0],w[1]-weapon_speed] for w in weapons]   # 무기 위치를 위로 옮기는 것
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        break  # 안쪽 for문에서 break를 만나면 여기로 진입 가능. 2중 for문을 한번에 
